...
export const Login = ({ onLogin, onShowToast }) => {
  ...
  const [rememberMe, setRememberMe] = useState(true); // default kecentang biar sesi otomatis keinget
  ...
  // Buka halaman login pake ?akses=guru (misal: yoursite.com/?akses=guru)
  // biar toggle "Login Sebagai" muncul & default-nya langsung ke Guru.
  // Tanpa query param ini, halaman login cuma nunjukin form siswa doang —
  // siswa gak bakal tau ada opsi login guru.
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    if (params.get("akses") === "guru") {
